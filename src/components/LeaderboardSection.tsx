    ...
    if (sortConfig.key === 'rank') {
      return [...traders].sort((a, b) => {
        ...
        if (sortConfig.direction === 'desc') {
          // Sort by absolute value first (higher absolute values first)
          const absA = Math.abs(rankChangeA);
          const absB = Math.abs(rankChangeB);

          // If one is positive and one is negative or zero
          if (rankChangeA > 0 && rankChangeB <= 0 || rankChangeA >= 0 && rankChangeB < 0) return -1;
          if (rankChangeB > 0 && rankChangeA <= 0 || rankChangeB >= 0 && rankChangeA < 0) return 1;

          // If both are positive or both are negative, sort by value
          return rankChangeB - rankChangeA;
        } else {
          // Ascending order - reverse the logic
          if (rankChangeA < 0 && rankChangeB >= 0 || rankChangeA <= 0 && rankChangeB > 0) return -1;
          if (rankChangeB < 0 && rankChangeA >= 0 || rankChangeB <= 0 && rankChangeA > 0) return 1;
      ...
      <div className="h-full overflow-y-auto custom-scrollbar space-y-4 pb-4">
        ...
        y: 0
          ...
          <div className="flex flex-col gap-3 mt-24 my-0">
            <div className="flex items-center gap-2 overflow-x-auto custom-scrollbar pb-2 mt-32 mx-0 my-[69px]">
              ...
              <Button variant="ghost" size="sm" onClick={() => onSort('roi')} className={cn("text-xs font-mono whitespace-nowrap", sortConfig.key === 'roi' && "bg-emerald-500/10 text-emerald-400")}>
                ROI {sortConfig.key === 'roi' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
              </Button>
              <Button variant="ghost" size="sm" onClick={() => onSort('score')} className={cn("text-xs font-mono whitespace-nowrap", sortConfig.key === 'score' && "bg-emerald-500/10 text-emerald-400")}>
        ...
        {sortedTraders.map((trader, index) => <motion.div key={trader.trader} initial={{
        opacity: 0,
        x: -20
      }} animate={{
        opacity: 1,
        ...
        delay: index * 0.1
            ...
            <TraderCard trader={trader.trader} score={trader.score} status={trader.status} position={index + 1} />
          </motion.div>)}
      </div>
    </div>;
};
