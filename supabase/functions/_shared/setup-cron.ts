...
/**
 * Setup a cron job to run an edge function every minute
 */
export async function setupCronJob() {
  try {
    ...
    // Create or replace the cron job to run every minute
    ...
    return { success: true, message: 'Cron job setup successfully' };
  } catch (error) {
